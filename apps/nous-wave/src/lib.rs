...
impl NousRuntime {
    pub async fn materialize(
        ...
        self.store
            ...
            .await?;
        let resource = match &request.reference {
            CognitiveRef::Resource(resource) => Some(resource.clone()),
            CognitiveRef::ExternalObject(_) => request.resource.clone(),
            _ => None,
        };
        let Some(resource) = resource else {
            return self.material.materialize(subject, request).await;
        };
        let handle = request
            .resource_handle
            .as_deref()
            .ok_or_else(|| Error::Invalid("Resource materialization handle is required".into()))?;
        let source = self
            .cognition
            .materialize_resource(subject, &resource, handle)
            .await?;
        let total = source.bytes.len() as u64;
        let range = request
            .byte_range
            .unwrap_or(nous_material_service::ByteRange {
                start: 0,
                end: total,
            });
        if request.max_bytes == 0 || range.end < range.start || range.end > total {
            return Err(Error::Invalid("invalid resource byte range/bound".into()));
        }
        let end = range.end.min(range.start.saturating_add(request.max_bytes));
        Ok(nous_material_service::MaterializedEvidence {
            reference: request.reference.clone(),
            media_type: source.media_type,
            bytes: source.bytes[range.start as usize..end as usize].to_vec(),
            byte_range: nous_material_service::ByteRange {
                start: range.start,
                end,
            },
            total_bytes: total,
            partial: range.start > 0 || end < total,
            provenance: vec![request.reference, CognitiveRef::Resource(resource)],
            producer: None,
            selection: None,
        })
    }
