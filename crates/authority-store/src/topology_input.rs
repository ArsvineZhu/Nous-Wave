...
fn link(from: CognitiveRef, to: CognitiveRef, kind: &str) -> TopologyEdgeSource {
    TopologyEdgeSource { from, to, support_class: "memory_evidence".into(), association_kind: kind.into(), polarity: "positive".into(), support_value: 1.0, bridge_hint: false }
}

impl AuthorityStore {
    pub async fn topology_projection_input(&self, subject: SubjectId, memory_enabled: bool) -> Result<TopologyProjectionInput> {
        ...
        if memory_enabled {
            ...
            for source in sources {
                ...
                for tag in source.tag_ids {
                    let tag = CognitiveRef::Tag(TagId(tag.parse().map_err(|_| Error::Infrastructure("invalid Tag id".into()))?));
                ...
                for entity in source.entity_refs {
                    ...
                    edges.push(link(source.reference.clone(), entity.clone(), "experiential"));
            ...
            for row in rows {
                let from = parse_reference(&row.try_get::<String,_>("from_ref_kind").map_err(db)?, &row.try_get::<String,_>("from_ref").map_err(db)?)?;
                let to = parse_reference(&row.try_get::<String,_>("to_ref_kind").map_err(db)?, &row.try_get::<String,_>("to_ref").map_err(db)?)?;
                if !allowed(&from) || !allowed(&to) { continue; }
                nodes.insert(from.clone()); nodes.insert(to.clone());
                edges.push(TopologyEdgeSource { from, to, support_class: row.try_get("support_class").map_err(db)?, association_kind: row.try_get("association_kind").map_err(db)?, polarity: row.try_get("polarity").map_err(db)?, support_value: row.try_get("support_value").map_err(db)?, bridge_hint: row.try_get("bridge_hint").map_err(db)? });
            }
            ...
            for row in rows {
                ...
                let mut support = parse_reference(&row.try_get::<String,_>("support_ref_kind").map_err(db)?, &row.try_get::<String,_>("support_ref").map_err(db)?)?;
                if let CognitiveRef::MemoryRevision(revision) = support {
                    let memory: Uuid = sqlx::query_scalar("SELECT memory_id FROM memory_revisions WHERE memory_revision_id=$1").bind(revision.0).fetch_one(&mut *tx).await.map_err(db)?;
                    support = CognitiveRef::Memory(MemoryId(memory));
                }
                if !allowed(&support) { continue; }
                nodes.insert(anchor.clone()); nodes.insert(support.clone());
                edges.push(link(anchor.clone(), support.clone(), "anchor")); edges.push(link(support, anchor, "anchor"));
            }
            let rows = sqlx::query("SELECT l.memory_id AS from_memory,r.memory_id AS to_memory,rr.relation FROM memory_revision_relations rr JOIN memory_revisions l ON l.memory_revision_id=rr.from_revision_id JOIN memory_revisions r ON r.memory_revision_id=rr.to_revision_id JOIN memory_objects lo ON lo.memory_id=l.memory_id JOIN memory_objects ro ON ro.memory_id=r.memory_id WHERE lo.subject_id=$1 AND ro.subject_id=$1 AND lo.status='active' AND ro.status='active'")
                .bind(subject.0).fetch_all(&mut *tx).await.map_err(db)?;
            for row in rows {
                edges.push(link(CognitiveRef::Memory(MemoryId(row.try_get("from_memory").map_err(db)?)), CognitiveRef::Memory(MemoryId(row.try_get("to_memory").map_err(db)?)), &row.try_get::<String,_>("relation").map_err(db)?));
            }
        }
        for resource in sqlx::query_scalar::<_, String>("SELECT resource_ref FROM resources WHERE subject_id=$1").bind(subject.0).fetch_all(&mut *tx).await.map_err(db)? { nodes.insert(CognitiveRef::Resource(ResourceRef::new(resource)?)); }
        let mut nodes: Vec<_> = nodes.into_iter().collect(); nodes.sort_by_key(ToString::to_string);
        tx.commit().await.map_err(db)?;
        Ok(TopologyProjectionInput { watermark, nodes, edges })
    }
}

fn allowed(reference: &CognitiveRef) -> bool {
    matches!(reference, CognitiveRef::Memory(_) | CognitiveRef::Tag(_) | CognitiveRef::Anchor(_) | CognitiveRef::Entity(_) | CognitiveRef::Resource(_))
}
