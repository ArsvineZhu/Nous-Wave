...
pub(crate) async fn watermark(tx: &mut Transaction<'_, Postgres>, subject: SubjectId, family: &str, space: &str) -> Result<i64> {
...
impl AuthorityStore {
    pub async fn projection_watermark(&self, subject: SubjectId, family: &str, space: &str) -> Result<i64> {
        let mut tx = self.begin().await?;
        watermark(&mut tx, subject, family, space).await
    }

    pub async fn text_projection_input(&self, subject: SubjectId, family: &str, space: &str, memory_enabled: bool) -> Result<TextProjectionInput> {
        let mut tx = self.begin().await?;
        ...
        if memory_enabled { sources.extend(memory_sources(&mut tx, subject).await?); }
...
pub(crate) async fn memory_sources(tx: &mut Transaction<'_, Postgres>, subject: SubjectId) -> Result<Vec<TextProjectionSource>> {
    ...
    for row in rows {
        ...
        sources.push(TextProjectionSource {
            reference: CognitiveRef::Memory(MemoryId(row.try_get("memory_id").map_err(db)?)), revision: Some(MemoryRevisionId(revision)),
            text: Some(row.try_get("representation_text").map_err(db)?), content_hash: None, title: row.try_get("title").map_err(db)?, media_type: "text/plain".into(),
            source_class: None, source_region: None, entity_refs: entities, tag_ids: tags, anchor_ids: anchors,
        });
    }
    for (table, sql) in [("tags", "SELECT o.tag_id AS id,r.label FROM tags o JOIN tag_revisions r ON r.tag_revision_id=o.current_revision_id WHERE o.subject_id=$1 AND o.status='active'"), ("anchors", "SELECT o.anchor_id AS id,r.description AS label FROM anchors o JOIN anchor_revisions r ON r.anchor_revision_id=o.current_revision_id WHERE o.subject_id=$1 AND o.status='active'")] {
        for row in sqlx::query(sql).bind(subject.0).fetch_all(&mut **tx).await.map_err(db)? {
            let id: Uuid = row.try_get("id").map_err(db)?;
            let reference = if table == "tags" { CognitiveRef::Tag(TagId(id)) } else { CognitiveRef::Anchor(AnchorId(id)) };
            sources.push(TextProjectionSource { reference, revision: None, text: Some(row.try_get("label").map_err(db)?), content_hash: None, title: None, media_type: "text/plain".into(), source_class: None, source_region: None, entity_refs: Vec::new(), tag_ids: Vec::new(), anchor_ids: Vec::new() });
        }
    }
    Ok(sources)
}

async fn material_sources(tx: &mut Transaction<'_, Postgres>, subject: SubjectId) -> Result<Vec<TextProjectionSource>> {
    ...
    for row in rows {
        ...
        sources.push(TextProjectionSource { reference: CognitiveRef::Occurrence(OccurrenceId(occurrence)), revision: None, text: None, content_hash: row.try_get("content_hash").map_err(db)?, title: None, media_type: row.try_get::<Option<String>,_>("media_type").map_err(db)?.unwrap_or_else(|| "application/octet-stream".into()), source_class: Some(row.try_get("source_class").map_err(db)?), source_region: region.map(SourceRegionId), entity_refs: entities, tag_ids: Vec::new(), anchor_ids: Vec::new() });
    }
    ...
    for row in rows {
        ...
        sources.push(TextProjectionSource { reference: CognitiveRef::DerivedRepresentation(DerivedRepresentationId(row.try_get("derived_representation_id").map_err(db)?)), revision: None, media_type: if text.is_some() { "text/plain".into() } else { row.try_get::<Option<String>,_>("media_type").map_err(db)?.unwrap_or_else(|| "application/octet-stream".into()) }, text, content_hash: row.try_get("content_hash").map_err(db)?, title: Some(row.try_get("representation_kind").map_err(db)?), source_class: Some("derived".into()), source_region: Some(SourceRegionId(row.try_get("source_region_id").map_err(db)?)), entity_refs: Vec::new(), tag_ids: Vec::new(), anchor_ids: Vec::new() });
    }
    Ok(sources)
}
